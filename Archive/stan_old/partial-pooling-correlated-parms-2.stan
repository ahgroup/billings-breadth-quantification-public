...
parameters {
	...
	// Partial/adaptive pooling means
	real lambda_a;
	...
	vector[2] AB_vec;
	// They also get their own correlation matrix?
	cholesky_factor_corr[2] ABcorr;
...
model {
	...
	ABcorr ~ lkj_corr_cholesky(1);
	s ~ exponential(1);

	// Now sample the lambdas which I called a and b here from an MVN dist
	ab_vec ~ mutli_normal_cholesky([0, 0]', diag_pre_multiply(s_lamb, ABcorr));
